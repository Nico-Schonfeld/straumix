...
// Tipo para las propiedades responsivas
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const configWithResponsive = config as Partial<GradualBlurProps> &
        ResponsiveProps;

      if (
        w <= 480 &&
        configWithResponsive[`mobile${k}` as keyof ResponsiveProps]
      ) {
        v = configWithResponsive[`mobile${k}` as keyof ResponsiveProps];
      } else if (
        w <= 768 &&
        configWithResponsive[`tablet${k}` as keyof ResponsiveProps]
      ) {
        v = configWithResponsive[`tablet${k}` as keyof ResponsiveProps];
      } else if (
        w <= 1024 &&
        configWithResponsive[`desktop${k}` as keyof ResponsiveProps]
      ) {
        v = configWithResponsive[`desktop${k}` as keyof ResponsiveProps];
      }
      setVal(v);
    };
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const config = useMemo(() => {
    ...
    const mergedConfig = mergeConfigs(DEFAULT_CONFIG, presetConfig, props);
    return mergedConfig as Required<
      Omit<
        GradualBlurProps,
        "children" | "onAnimationComplete" | "className" | "style"
      >
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, isVisible, props.style]);
  ...
  }, [isVisible, config.animated, config.duration, props.onAnimationComplete]);
...
// Extender el tipo para incluir las propiedades estáticas
interface GradualBlurMemoType
  extends React.MemoExoticComponent<React.FC<GradualBlurProps>> {
  PRESETS: typeof PRESETS;
  CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS;
}

(GradualBlurMemo as GradualBlurMemoType).PRESETS = PRESETS;
(GradualBlurMemo as GradualBlurMemoType).CURVE_FUNCTIONS = CURVE_FUNCTIONS;
