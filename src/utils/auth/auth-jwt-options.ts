...
export async function encrypt(payload: Payload): Promise<string> {
  return await new SignJWT(payload)
    ...
    .setIssuedAt()
    .setExpirationTime("24h") // cambiado a 24 horas
    .sign(key);
}
...
export async function login(formData: FormData) {
  ...
  const expires = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 horas en milisegundos
...
export async function updateSession(req: NextRequest) {
  ...
  parsed.expires = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 horas en milisegundos
